...
public sealed class AutenticacionController : ControladorBase
{
    ...
    public async Task<IActionResult> IniciarSesion([FromBody] IniciarSesionComando comando)
    {
    ...
    [AllowAnonymous]
    [Produces("application/json")]
    public async Task<IActionResult> RenovarToken([FromBody] RenovarTokenComando comando)
    {
        var r = await Mediator.Send(comando);
        return StatusCode(r.StatusCode, r);
    }
    ...
    public async Task<IActionResult> CerrarSesion()
    {
        ...
        var r = await Mediator.Send(new CerrarSesionComando(token));
        return StatusCode(r.StatusCode, r);
    }
    ...
    public async Task<IActionResult> ObtenerPerfil()
    {
        var idUsuario = int.Parse(
            ...
            ?? "0");
        var r = await Mediator.Send(new ObtenerPerfilConsulta(idUsuario));
        return StatusCode(r.StatusCode, r);
    }
}
