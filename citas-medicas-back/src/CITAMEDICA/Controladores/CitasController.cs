...
public sealed class CitasController : ControladorBase
{
    ...
    public async Task<IActionResult> Obtener([FromQuery] ObtenerCitasConsulta consulta)
    {
    ...
    [HttpPost]
    [Produces("application/json")]
    public async Task<IActionResult> Registrar([FromBody] RegistrarCitaComando comando)
    {
    ...
    [HttpPut("{id:int}")]
    [Produces("application/json")]
    public async Task<IActionResult> Actualizar(int id, [FromBody] ActualizarCitaComando comando)
    {
    ...
    {
        var r = await Mediator.Send(new CancelarCitaComando(id));
        return StatusCode(r.StatusCode, r);
    }
}
