...
public sealed class PacientesController : ControladorBase
{
    ...
    public async Task<IActionResult> ObtenerTodos([FromQuery] ObtenerPacientesVM consulta)
    {
    ...
    [HttpPut("{id:int}")]
    [Produces("application/json")]
    public async Task<IActionResult> Actualizar(int id, [FromBody] ActualizarPacienteVM comando)
    {
    ...
    {
        var r = await Mediator.Send(new EliminarPacienteVM(id));
        return StatusCode(r.StatusCode, r);
    }
}
